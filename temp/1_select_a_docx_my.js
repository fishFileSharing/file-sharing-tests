...
describe("Select a file and check if it's name appears on page", function () {
  ...
  it("should open https://file-sharing-dev.netlify.app/ and check for title", async function () {
    ...
    const filePath = path.join(__dirname, "..", "test_files", "docx.docx")
    // Создаем абсолютный путь к файлу `docx.docx`, который находится в папке `test_files`.
    ...
    assert(
      selectedFileNameText.includes("docx", "Something is wrong with filename")
    )
